...
class MyHashSet
    ...
    def union(set2)
        new_union = @store.keys.concat(set2.to_a)
        set3 = MyHashSet.new
        new_union.each {|i|set3.insert(i)}
        set3
    end
    
    def intersect(set2)
        counter_intersect = @store.keys - set2.to_a
        new_intersect = @store.keys - counter_intersect
        set3 = MyHashSet.new
        new_intersect.each {|i|set3.insert(i)}
        set3
    end
    
    def minus(set2)
        new_minus = @store.keys - set2.to_a
        set3 = MyHashSet.new
        new_minus.each {|i|set3.insert(i)}
        set3
    end
end
