...
class Array
  def my_uniq
    uniq_arr = []
    self.each {|i| uniq_arr << i unless uniq_arr.include?(i)}
    uniq_arr
  end
end
  ...
  def two_sum
    ...
    while i < self.length do
      while j < self.length do
        positions << [i,j] if (self[i] + self[j]) == 0
        j += 1
      end
    i += 1
    j = i + 1
...
class Array
  def median
    ...
    self.sort!
    return self[(self.length / 2).floor] if self.length % 2 == 1
